...
Out = gzip.open(r'ENSP2EntrezSym_DB.txt.gz', 'wb')
ENSP2EntrezSym_DB = {}
with gzip.open(r'10090.protein.aliases.v10.txt.gz', 'rb') as f:
	for line in islice(f, 1, None):
		...
		if 'Ensembl_EntrezGene' in line[2]:
			if ENSP2EntrezSym_DB.has_key(line[0] ):
				ENSP2EntrezSym_DB[line[0] ].append(line[1] )
			else:
				ENSP2EntrezSym_DB[line[0] ] = [line[1] ]
				
			Out.writelines('\t'.join(line[:2] ) )
			Out.write('\n')
			
with open(r'ENSP2EntrezSym_DB.pickle', 'wb') as f:
	dump(ENSP2EntrezSym_DB, f)
