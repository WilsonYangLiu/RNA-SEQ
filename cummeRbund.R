...
cuff_data  <- readCufflinks("diff_Sb-2w-check")		# or "diff_off-2w"
...
mygene
png(filename = paste(gene_of_interest, ".png", seq=""))
...
png(filename = paste(gene_of_interest,"isoform", ".png", seq=""))
...
nrow(sig_GeneData)
write.table(sig_GeneData, 'sig_GeneData.txt', sep='\t', row.names=F, col.names=T, quote=F)
## Record genes with q value less < 0.1
q0.1_GeneData <- subset(gene_diff_data, (q_value < 0.1))
print("number of genes (q_value < 0.1): ")
nrow(q0.1_GeneData)
write.table(q0.1_GeneData, 'q0.1_GeneData.txt', sep='\t', row.names=F, col.names=T, quote=F)
...
write.table(fold_change2_GeneData, 'fold_change2_GeneData.txt', sep='\t', row.names=F, col.names=T, quote=F)
